...
OAuth 2.1 authentication for [MCPHero](https://github.com/atomicbi/mcphero) MCP servers. Acts as an OAuth proxy between MCP clients and upstream identity providers (Google, etc.), handling PKCE, refresh tokens, dynamic client registration (CIMD), and protected resource metadata (RFC 9728).
...
```typescript
import { google, createJsonStore } from '@mcphero/auth'
...
```typescript
import { createOAuthProxy } from '@mcphero/auth'
...
```typescript
import { validateToken } from '@mcphero/auth'
...
```typescript
import { generateProtectedResourceMetadata } from '@mcphero/auth'
...
```typescript
import { createMemoryStore } from '@mcphero/auth'
...
```typescript
import { createJsonStore } from '@mcphero/auth'
...
```typescript
import { createRedisStore } from '@mcphero/auth'
import Redis from 'ioredis'

const store = createRedisStore({
  client: new Redis(),
  prefix: 'mcphero:oauth:'  // default prefix
})
```
...
```typescript
import type { OAuthStore } from '@mcphero/auth'
